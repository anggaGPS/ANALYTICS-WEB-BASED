...
echo"<SCRIPT LANGUAGE='Javascript' SRC='FusionCharts/FusionCharts.js'></SCRIPT>";
 ...
 $strXML="<graph caption='Grafik Penjualan' numberPrefix='  ' yAxisName='jumlah' decimalPrecision='0' formatNumberScale='0'>";
	...
	while($Data=mysql_fetch_array($qr)){
   	...
   	$arrData[0][2]="$Data[jumlah2015]";
   
   	$arrData[0][3]="$Data[jumlah2016]";
	
    $arrData[0][4]="$Data[jumlah2017]";
	   	
	$arrData[0][5]="$Data[jumlah2018]";
   
   	$arrData[0][6]="$Data[jumlah2019]";
	
    $arrData[0][7]="$Data[jumlah2020]";
	
   	$arrData[0][8]="$Data[jumlah2021]";
   
   	$arrData[0][9]="$Data[jumlah2022]";
	
    $arrData[0][10]="$Data[jumlah2023]";
...
$min_tahun=mysql_fetch_array(mysql_query("select min(tahun) as min_tahun from view_analisis"));
$max_tahun=mysql_fetch_array(mysql_query("select max(tahun) as max_tahun from view_analisis"));
?>

<form action="?page=grafikselisih" method="post" name="postform">
...
if(isset($_POST['cari'])){
	...
	if(empty($provinsi) and empty($tahun_awal) and empty($tahun_akhir)){
		...
		$jumlah=mysql_fetch_array(mysql_query("select sum(jumlah) as total from view_analisis"));
		...
		$query=mysql_query("select * from view_analisis where provinsi like '%$provinsi%' and tahun between '$tahun_awal' and '$tahun_akhir'");
		$jumlah=mysql_fetch_array(mysql_query("select sum(jumlah) as total from view_analisis where provinsi like '%$provinsi%' and tahun between '$tahun_awal' and '$tahun_akhir'"));
	}
            ...
            <div class="col-xs-12">
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title">Data Target Belum Tercapai</h3>
<div class="box-body">
 <td> <a href="exportgrafikselisih.php" class="btn btn-info btn-flat">Export Data</a></td>
	...
	?>
    <tr>
    	<td colspan="3" align="right"><strong>TOTAL T</strong></td><td align="center"><?php echo number_format($jumlah['total'],0,',','.');?></td>
    </tr>
