...
$min_tahun=mysql_fetch_array(mysql_query("select min(tahun) as min_tahun from view_target"));
$max_tahun=mysql_fetch_array(mysql_query("select max(tahun) as max_tahun from view_target"));
?>

<form action="?page=grafikbanding" method="post" name="postform">
...
if(isset($_POST['cari'])){
	...
	if(empty($provinsi) and empty($tahun_awal) and empty($tahun_akhir)){
		...
		$jumlah=mysql_fetch_array(mysql_query("select sum(target) as total from view_target"));
		$jumla=mysql_fetch_array(mysql_query("select sum(jumlah) as total from view_target"));
		...
		$query=mysql_query("select * from view_target where provinsi like '%$provinsi%' and tahun between '$tahun_awal' and '$tahun_akhir'");
		$jumlah=mysql_fetch_array(mysql_query("select sum(target) as total from view_target where provinsi like '%$provinsi%' and tahun between '$tahun_awal' and '$tahun_akhir'"));
		$jumla=mysql_fetch_array(mysql_query("select sum(jumlah) as total from view_target where provinsi like '%$provinsi%' and tahun between '$tahun_awal' and '$tahun_akhir'"));
...
<div class="box-body">
<tr>
    <td> <a href="exportbanding.php" class="btn btn-info btn-flat">Export Data</a></td>
    <td colspan="2">&nbsp;</td>
</tr>
...
echo"<SCRIPT LANGUAGE='Javascript' SRC='FusionCharts/FusionCharts.js'></SCRIPT>";
 ...
 $strXML="<graph caption='Grafik Penjualan' numberPrefix='  ' yAxisName='peserta' decimalPrecision='0' formatNumberScale='0'>";
	...
	$sql=("SELECT * FROM view_target where provinsi like '%$provinsi%' and tahun between '$tahun_awal' and '$tahun_akhir'"); $qr=mysql_query($sql); while($Data=mysql_fetch_array($qr)){
