				...
				$.ajax({
					...
					success: function(data) {							
						...
						for(var i=0;i<data.provinsi.length;i++)
						{
							...
							jumlah2017=data.provinsi[i].jumlah2017;
							kode=data.provinsi[i].kode;
							if(jumlah2017<=0)
							{
								min+=kode+",";							
							}
							else if(jumlah2017>0&&jumlah2017<3000)
							{
								med+=kode+",";
							}
							else if(jumlah2017>=3000)
							{
								max+=kode+",";
							}							
						}					
					...
					success: function(data) {
						...
						for(var i=0;i<data.provinsi.length;i++)
						{
							if(data.provinsi[i].kode==kodeBPS)
							{
								...
								isi+="<b>Target Yang Belum Tercapai : </b>"+data.provinsi[i].jumlah2017+" orang</br>";
								$('#showdata').html(isi);
...
$result = mysql_query("select * from view_jumlah2017");  
...
while ($row = mysql_fetch_array($result)) {  
    ...
    $jsArray .= "tahun['" . $row['tahun'] . "'] = '" . addslashes($row['jumlah2017']) . "';\n";  
}  
echo '</select>';  
?>  <br></br>
